...
def cpu_efficiency_scaled_by_jobslots(df, include_zero_cpu=False):
    ...
    # total_walltime = df_filtered[wrap_wc].dot(df_filtered[core_count])
    total_walltime = (df_filtered[wrap_wc] * df_filtered[core_count] * df_filtered['coresLogical'] / df_filtered[
        'jobslots']).sum()
...
def cpu_efficiency_scaled_by_jobslots_physical(df, include_zero_cpu=False):
    ...
    # total_walltime = df_filtered[wrap_wc].dot(df_filtered[core_count])
    total_walltime = (
                df_filtered[wrap_wc] * df_filtered[core_count] * df_filtered['cores'] / df_filtered['jobslots']).sum()
